...
void calc_w_k2n(std::vector<Tree*>& trees) {
	...
	for (size_t i = 0; i < trees.size(); i++) {
		...
		for (size_t j = 0; j < tree->get_nodes_num(); j++) {
			...
			cache_size[pos] = tree->get_node(j)->leaf_size;
			pos++;
		}
	}
...
bool is_special(Tree::Node* node) {
	return node->leaf_size < orig_t2->get_node(node->secondary_id)->leaf_size;
}
void filter_clusters_nlog2n(Tree::Node* t1_root, Tree* tree2, taxas_ranges_t* t1_tr, lca_t* t2_lcas, lca_t* orig_t2_lcas,
	...
	while (curr != t1_root->parent) {
		...
		if (counter[rim1->id] == rim1->leaf_size && ri != rim1 && rim1->get_children_num() != 1) {
			// if children of rim1 are all "marked", should not go into BT
			counter[rim1->parent->id] += rim1->leaf_size;
		...
		for (int i = curr_t1_start; i <= curr_t1_end; i++) {
			Tree::Node* x = tree2->get_leaf(t1_tr->taxas[i]);
			counter[x->id]++;
			while (x != ri && counter[x->id] == x->leaf_size && !is_special(x)) { // TODO: check
				counter[x->parent->id] += x->leaf_size;
				BT[x->id] = false;
				x = x->parent;
			}
		}
...
Tree* freqdiff(std::vector<Tree*>& trees, bool centroid_paths = true) {
	...
	for (size_t i = 1; i < trees.size(); i++) {
		...
		auto ed = Now();
		// cout << "one-time: " << duration_sec(st, ed) << endl;
		// exit(0);
		...
		// 	cout << to_del_ti[i] << ' '; cout << '\n';
		// // for (int i = 0; i < T->get_nodes_num(); i++)
		// // 	cout << to_del_t[i] << ' '; cout << '\n';
		// exit(0);
